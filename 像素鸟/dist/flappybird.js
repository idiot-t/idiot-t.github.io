(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.FlappyBird = factory());
  ...
  function isPC() { 
    ...
    for (var v = 0; v < Agents.length; v++) {
      // 如果 userAgent 字符串里面包含 Agents 数组里面的单词，那么就得出判断不是pc端 
  ...
  Shape.prototype.draw = function draw (name, x, y, width, height) {
    // name 为需要画的图象的名称
    // x,y 为画图的坐标
    // width 和 height 为图的宽高
    var pos = images[name];// 得到对应图片的坐标信息
    var sx = pos.x;
    var sy = pos.y;
    var sWidth = pos.w;
    var sHeight = pos.h;
  ...
  var Bird = function Bird(gameIns) {
    this.gameIns = gameIns; // 存储 game实例
    this.shape = gameIns.shape; // 存储 game实例上的shape对象
    // 小鸟的宽高
    this.width = 34;
    this.height = 24;
    //小鸟的坐标
    ...
    this.g = 1; // 重力
    this.rotate = 0;// 小鸟的旋转角度

    this.timer = null;
    this.timer2 = null;
  };

  // 先调整画布的位置以便按照 rotate 画出倾斜的小鸟

  Bird.prototype.draw = function draw () {
    this.gameIns.ctx.save();

    // 下面三条语句相当于调整了画布的角度，其他的并没有改变
    ...
    this.gameIns.ctx.restore();// 画完鸟儿就把画布归位
  };
  ...
  Bird.prototype.gravity = function gravity () {
    this.g *= 1.05;
    this.top += this.g;
    if (this.rotate < 60) { // 旋转角度低于60度则，增加旋转角度
      if (this.rotate < 10) {
        this.rotate += 2;
      } else {
        this.rotate += 1.3;
  ...
  var Game = function Game() {
    ...
    // canvas.width 和 canvas.height 才是画布的实际大小
  ...
  Game.prototype.init = function init () {
    ...
    this.img.src = 'source/flappybird.png';
      ...
      // 给game 实例添加一个 shape实例， shape实例上有 ctx,img属性 还有一个 draw() 方法
  ...
  // 给 game 实例添加一个处理开始游戏的 处理函数属性， 给画布 canvas 添加这个处理函数
  Game.prototype.coverListener = function coverListener () {
      ...
      var p = getEventPosition(theEvent);
  			...
  			if(p.x > this$1.btnX && p.x < this$1.btnX + 116 && p.y > this$1.btnY && p.y < this$1.btnY + 70) {// 如果点击了按钮则开始游戏
  				this$1.isControl = true;
        this$1.bird = new Bird(this$1);// 实例化一个 bird对象，并添加到 game 实例上
        this$1.s_click.play(); // 播放点击音效
          
        removeEvent(this$1.canvas, this$1.eventType.start, this$1.handler2); //点击之后就移除该按钮点击

        this$1.ctx.clearRect(0, 0, this$1.width, this$1.height);// 清空屏幕
        this$1.drawBg();
  ...
  Game.prototype.run = function run () {
    ...
    // 绘制鸟
    this.bird.draw();
    this.bird.gravity();// 更新在重力作用下下一次鸟的位置信息
  ...
  //开始循环生成管道
  	Game.prototype.startPipe = function startPipe () {
  		...
  		this.pipe_timer = setInterval(function () {
  			...
  			this$1.drawPipe(id); // ID 是一个管道的标识符
  		}, this.gapPipe);	
  };
